# Configure paths for SDL3
# Based on SDL2 detection macro
...
dnl Test for SDL3, and define SDL3_CFLAGS and SDL3_LIBS
dnl
AC_DEFUN([AM_PATH_SDL3],
[dnl 
dnl Get the cflags and libraries from the sdl3-config script
dnl
AC_ARG_WITH(sdl-prefix,[  --with-sdl-prefix=PFX   Prefix where SDL3 is installed (optional)],
            sdl_prefix="$withval", sdl_prefix="")
AC_ARG_WITH(sdl-exec-prefix,[  --with-sdl-exec-prefix=PFX Exec prefix where SDL3 is installed (optional)],
            sdl_exec_prefix="$withval", sdl_exec_prefix="")
AC_ARG_ENABLE(sdltest, [  --disable-sdltest       Do not try to compile and run a test SDL3 program],
            , enable_sdltest=yes)
...
AC_ARG_VAR(SDL3_FRAMEWORK, [Path to SDL3.framework])
  ...
  if test "x$sdl_prefix$sdl_exec_prefix" = x ; then
    PKG_CHECK_MODULES([SDL3], [sdl3 >= $min_sdl_version],
           [sdl_pc=yes],
           [sdl_pc=no])
  else
    sdl_pc=no
  ...
  if test "x$sdl_pc" = xyes ; then
    ...
    if test "$SDL3_CONFIG" = "no" -a "x$search_sdl_framework" = "xyes"; then
      ...
      if test "x$SDL3_FRAMEWORK" != x; then
        ...
        for d in / ~/ /System/; do
          if test -d "$dLibrary/Frameworks/SDL3.framework"; then
            sdl_framework="$dLibrary/Frameworks/SDL3.framework"
          fi
        done
      fi
      ...
      if test -d $sdl_framework; then
        ...
        SDL3_CFLAGS="-F$sdl_framework_dir -Wl,-framework,SDL3 -I$sdl_framework/include"
        SDL3_LIBS="-F$sdl_framework_dir -Wl,-framework,SDL3"
      else
        no_sdl=yes
      fi
    fi
    ...
    if test "$SDL3_CONFIG" != "no"; then
      ...
      if test "x$enable_sdltest" = "xyes" ; then
        ...
        LIBS="$LIBS $SDL3_LIBS"
dnl
dnl Now check if the installed SDL3 is sufficiently new. (Also sanity
...
int main (int argc, char *argv[])
{
  ...
  */
  ...
  if (sscanf(tmp_version, "%d.%d.%d", &major, &minor, &micro) != 3) {
   ...
   if (($sdl_major_version > major) ||
  ...
  else
    {
      ...
      printf("*** of SDL3 required is %d.%d.%d. If sdl3-config is correct, then it is\n", major, minor, micro);
  ...
  if test "x$no_sdl" = x ; then
     ...
     if test "$SDL3_CONFIG" = "no" ; then
       echo "*** The sdl3-config script installed by SDL3 could not be found"
       echo "*** If SDL3 was installed in PREFIX, make sure PREFIX/bin is in"
       ...
       if test -f conf.sdltest ; then
        :
          ...
          echo "*** Could not run SDL3 test program, checking why..."
          CFLAGS="$CFLAGS $SDL3_CFLAGS"
          CXXFLAGS="$CXXFLAGS $SDL3_CFLAGS"
          LIBS="$LIBS $SDL3_LIBS"
...
#define main K_and_R_C_main
          ...
          echo "*** that the run-time linker is not finding SDL3 or finding the wrong"
          echo "*** version of SDL3. If it is not finding SDL3, you'll need to set your"
      ...
      echo "***"
          ...
          echo "*** exact error that occured. This usually means SDL3 was incorrectly installed"
          echo "*** or that you have moved SDL3 since it was installed. In the latter case, you"
